...
async function seedUsers(count) {
  ...
  const hasPassword = await bycriptjs.hash("123456789",10)
  for (let i = 0; i < count; i++) {
    let uniqueEmail;
    do {
    ...
    await prisma.users.create({
      data: {
        ...
        isOwner: false,
        centreId: centreId, // Ensure this matches a valid centre ID
      },
    });
  }
}
  ...
  for (let i = 0; i < count; i++) {
    ...
    await prisma.students.create({
      data: {
        ...
        levelId: randomLevel.id,
        centreId: randomUser.id,
        subjects:{
            connect:getRandomElements(subjects,Math.floor(Math.random() * 5))
        }
      },
    });
  }
